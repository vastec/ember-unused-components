 ...
 *  - check if any directory has `component.js` or `template.hbs` file
 *
...
function _podsGuess(appPath) {
  ...
  let hasAtLeastOneComponent = false;
  ...
  try {
    ...
    files.forEach((item, index) => {
      ...
      if (stat.isDirectory()) {
        /*
          One level deep check if any of directories of `app/components` has `component.js` or `template.hbs`
         */
        ...
        dirPathFiles.forEach((dirItem, dirIndex) => {
          ...
          if (
            !dirStat.isDirectory() &&
            (dirPathFilename.includes('template.hbs') || dirPathFilename.includes('component.js'))
          ) {
