...
module.exports = {
  ...
  getConfig(commandOptions) {
    ...
    this.config = {
      ...
      ignore: [],
      includeAddons:
        commandOptions.includeAddons === undefined ? false : commandOptions.includeAddons || true,
      whitelist: [],
  ...
  getAppPaths: function(rootPath) {
    let searchPaths = [];
    if (_isAddon(rootPath)) {
      searchPaths = ['src', 'addon'];
    } else {
      searchPaths = ['src', 'app'];
    }
  ...
  getSearchPaths: function(rootPath, commandOptions) {
    ...
    if (podModulePrefix) {
      searchPathSuffixes.push(`app/${podModulePrefix}/${constants.DEFAULT_COMPONENTS_DIR_NAME}`);
    }
