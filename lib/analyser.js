    ...
    this.components.forEach(component => {
      ...
      if (hasCurlyBracesInvocation || hasAngleBracketsInvocation) {
        ...
        if (this.stats[component]) {
          this.stats[component].count++;

          if (hasCurlyBracesInvocation) {
            this.stats[component].curly++;
          } else {
            this.stats[component].angle++;
          }
        }
      }
    });
  ...
  logResults(showStats) {
    ...
    if (showStats) {
      ...
      console.log(
        ...
        curlyVsAngle.curly,
        colors.dim(`(${curlyVsAngle.curlyPercentage.toFixed(2)}%)`),
        colors.cyan('vs'),
        curlyVsAngle.angle,
        colors.dim(`(${curlyVsAngle.anglePercentage.toFixed(2)}%)`),
      );
    }
    console.log('');
  },
    ...
    files.forEach((item, index) => {
      ...
      if (stat.isDirectory()) {
        ...
        if (filename.includes(recognizer)) {
          ...
          this.stats[componentName] = { name: componentName, count: 0, curly: 0, angle: 0 };
        }
      }
    });
