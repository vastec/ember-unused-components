...
module.exports = {
  ...
  componentLookupInFile(filename, type) {
    ...
    this.components.forEach(component => {
      ...
      if (type === 'js') {
        lookupResult = lookup.componentLookupInJs(data, component.name);
      } else if (type === 'hbs') {
        lookupResult = lookup.componentLookupInHbs(data, component.name);
      }

      if (lookupResult) {
        let unusedRef = this.unusedComponents.find(u => u.key === component.key);
        let unusedIndex = this.unusedComponents.indexOf(unusedRef);
  ...
  isWhitelisted(componentOnWhitelist) {
    ...
    if (hasWildcard) {
      ...
      return this.unusedComponents.find(component => component.name === componentOnWhitelist);
    }
  },
  ...
  logResults(showStats, showOccurrences, whitelist) {
    ...
    if (this.unusedComponents.length > 0) {
      ...
      this.unusedComponents.forEach(component => console.log(`  - ${component.key}`));
    } else {
    ...
    if (showOccurrences) {
      ...
      for (const key of Object.keys(this.occurrences)) {
        if (!this.unusedComponents.find(u => u.key === key)) {
          console.log(colors.cyan(`\n  ${key}:`));
  ...
  mapComponents(config) {
    config.searchPaths.forEach(path => this.mapComponentPath(config, './' + path));
  },
    ...
    files.forEach((item, index) => {
      ...
      if (stat.isDirectory()) {
        ...
        if (_isComponent(config, filename)) {
          let component = _componentInfoFromPath(config, filename);
          if (!this.stats[component.key]) {
  ...
  scanProject(config) {
    config.appPaths.forEach(path => this.scanProjectPath(config, './' + path));
  },
...
function _isComponent(config, filename) {
  return (
    ['.js', '.hbs'].includes(path.extname(filename)) && path.basename(filename).indexOf('-test.js') === -1
  );
}
...
function _componentInfoFromPath(config, filename) {
  let addonName, addonSourcePath, componentName;

  if (config.includeAddons) {
    for (let addonPath of config.addonPaths) {
      if (filename.indexOf(addonPath) > -1) {
        addonSourcePath = addonPath;
        addonName = path.basename(addonPath);
        break;
      }
    }
  }
  ...
  for (let searchPath of config.searchPaths) {
    if (filename.indexOf(searchPath) > -1) {
      filename = filename.replace(searchPath, '');
      filename = filename.replace(/^[\\/\\.]+/, '');
    }
  }

  if (path.basename(filename) === 'component.js') {
    componentName = filename.replace('/component.js', '').trim();
  } else if (path.basename(filename) === 'template.hbs') {
    componentName = filename.replace('/template.hbs', '').trim();
  } else {
    componentName = filename.replace('.js', '').trim();
  }
  ...
  if (addonName) {
    ...
    info.source = addonName;
    info.sourcePath = addonSourcePath;
    info.key = `[${addonName}] ${componentName}`;
  } else {
    info.name = componentName;
    info.key = componentName;
  }
