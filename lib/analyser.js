...
module.exports = {
  ...
  mapComponentPath(config, pathToCheck) {
    console.log(`read dir: ${pathToCheck}`); 
...
function _isComponent(config, filename) {
  return filename.includes('/component.js') || filename.includes('.js');
}
...
function _componentInfoFromPath(config, filename) {
  ...
  if (config.includeAddons) {
    for (let addonPath of config.addonPaths) {
      if (filename.startsWith(addonPath)) {
  ...
  for (let searchPath of config.searchPaths) {
    if (filename.startsWith(searchPath)) {
