...
function _isComponent(config, filename) {
  return (
    ['.js', '.hbs'].includes(path.extname(filename)) &&
    path.basename(filename).indexOf('-test.js') === -1
  );
}
...
function _componentInfoFromPath(config, filename) {
  ...
  let componentName = _componentNameFromPath(config, filename);
  let info = { filePaths: [] };
  ...
  if (addonInfo) {
    ...
    info.key = `[${addonInfo.name}] ${componentName}`;
  } else {
    info.name = componentName;
...
/**
 * Returns component name from path
 *
 * @param {object} config
 * @param {array} config.addonPaths, config.searchPaths
 * @param {string} filename
 * @returns {object}
 * @private
 */

function _componentNameFromPath(config, filename) {
  let addonInfo = _addonInfoFromPath(config, filename);
  let componentName = filename;

  if (addonInfo) {
    componentName = _removeLeadingPath(componentName, path.join(addonInfo.sourcePath, 'addon'));
    componentName = _removeLeadingPath(componentName, addonInfo.sourcePath, 'addon');
  }

  config.searchPaths.forEach(searchPath => {
    if (componentName.indexOf(searchPath) > -1) {
      componentName = _removeLeadingPath(componentName, searchPath);
    }
  });

  componentName = _removeLeadingPath(componentName, 'components/', '/');
  componentName = _removeLeadingPath(componentName, 'templates/', '/');
...
function _addonInfoFromPath(config, filename) {
  let sourcePath, name;
