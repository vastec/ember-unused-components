...
module.exports = {
  ...
  logResults(showStats, showOccurrences, whitelist) {
    let percentage = (Object.keys(this.unusedComponents).length / Object.keys(this.components).length) * 100;
...
function _componentNameFromPath(config, filename) {
  ...
  let addonInfo = _addonInfoFromPath(config, filename);
  if (addonInfo) {
    componentName = filename.replace(path.join(addonInfo.sourcePath, 'addon'), '');
    componentName = componentName.replace(addonInfo.sourcePath, '');
  }

  config.searchPaths.forEach(searchPath => {
    if (componentName.indexOf(searchPath) > -1) {
      componentName = componentName.replace(searchPath, '');
