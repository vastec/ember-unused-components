...
module.exports = {
  ...
  _componentLookupInFile(filename) {
    ...
    this.components.forEach((component) => {
      ...
      if (data.match(re) && data.match(re).length > 0) {
  ...
  _config(commandOptions) {
    ...
    this.usePods = commandOptions.pods || typeof configFile.podModulePrefix === 'string';
  ...
  _mapComponents(pathToCheck) {
    ...
    files.forEach((item, index) => {
      ...
      if (stat.isDirectory()) {
        ...
        let recognizer = this.usePods ? '/component.js' : '.js';

        if (filename.includes(recognizer)) {
          let componentName = path.basename(filename.replace(recognizer, '').replace(componentsPath, ''));
